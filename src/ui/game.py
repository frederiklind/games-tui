...
class GameUI(object):
    ...
    def make_wins(self) -> None:
        ...
        self.__side_pane_l.bkgd(" ", curses.color_pair(8))  # set background color
        self.__side_pane_r.bkgd(" ", curses.color_pair(8))

        self.__side_pane_l.attron(curses.color_pair(9) | curses.A_BOLD)
        self.__side_pane_l.addstr(0, 0, " 󰆦 Moves          ")
        self.__side_pane_l.attroff(curses.color_pair(2) | curses.A_BOLD)

        self.__side_pane_r.attron(curses.color_pair(9) | curses.A_BOLD)
        self.__side_pane_r.addstr(0, 0, " 󰆦 Game           ")
        self.__side_pane_r.addstr(6, 0, " 󰔺 Highscore      ")
        self.__side_pane_r.attroff(curses.color_pair(2) | curses.A_BOLD)
        ...
        self.__cb_win.bkgd(" ", curses.color_pair(8))
        self.render_cube()
        self.render_arw()
    ...
    def increment_mv(self) -> None:
        ...
        self.__side_pane_r.addstr(3, 5, str(self.game.num_moves), curses.color_pair(10))
    ...
    def pos_y(self, side_idx: int) -> int:
        """
    ...
    def pos_x(self, side_idx: int) -> int:
        """
    ...
    def render_arw(self) -> None:
        ...
        for i in range(6):
            if i in [0, 1]:
                ...
                self.__cb_win.addstr(y, x, sa, curses.color_pair(11))
                self.__cb_win.addstr(y, x + 8, sb, curses.color_pair(11))
            else:
                y, x = self.__arw_xys[i]
                sa, sb = (a, b) if i == self.__idx else (" ", " ")
                self.__cb_win.addstr(y, x, sa, curses.color_pair(11))
                self.__cb_win.addstr(y + 4, x, sb, curses.color_pair(11))
        self.__cb_win.refresh()
            ...
            for r in range(len(cb[s])):
                ...
                for c in range(len(cb[s][r])):
                    self.__cb_win.addstr(
                        y + r, x + c, "", curses.color_pair(cb[s][r][c] + 11)
                    )
                    x += 1
        self.__cb_win.refresh()
            ...
            match i:
                case self.__idx:
                    self.__side_pane_l.addstr(
                        y + i, 2, f"<  {self.__opts[i]}  >", curses.color_pair(35)
                    )
                case _:
                    self.__side_pane_l.addstr(
                        y + i, 2, f"   {self.__opts[i]}    ", curses.color_pair(10)
                    )
            self.__side_pane_l.addstr(y + i, 4, "", curses.color_pair(i + 11))
    ...
    def render_key_cmd(self) -> None:
        ...
        for i in range(len(keys)):
            self.__side_pane_l.addstr(y + i, 2, symbols[i], curses.color_pair(35))
            self.__side_pane_l.addstr(y + i, 5, keys[i], curses.color_pair(10))
    ...
    def render_stats(self) -> None:
        ...
        """
        symbols = ["󰔛", "󱃴"]
        ph = [self.format_time(), "0"]
        y = 2
        for i in range(len(symbols)):
            self.__side_pane_r.addstr(y + i, 2, symbols[i], curses.color_pair(35))
            self.__side_pane_r.addstr(y + i, 5, ph[i], curses.color_pair(10))
