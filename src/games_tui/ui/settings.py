...
class SettingsUI(Window):
    ...
    def __init__(self, stdscr, height: int, width: int) -> None:
        ...
        self.__vals = [
            ...
            str(settings.sound_effects_enabled),
        ]
    ...
    def make_win(self) -> None:
        ...
        self.win.addstr(9, 0, " ".join([""] * (self.width + 1)))
        self.win.addstr(0, 1, "  App Settings")
        self.win.addstr(9, 1, "  Preview")
    ...
    def render_sample_win(self) -> None:
        """ """
        for i in range(10, self.height - 1):
            self.win.addstr(
                i, 0, " ".join([""] * (self.width - 1)), curses.color_pair(1)
            )
    ...
    def render_key_layout(self) -> None:
        ...
        if self.__vals[4] == "Vim":
            ...
            self.win.addstr(22, 56, arws[0], curses.color_pair(11))
            self.win.addstr(24, 65, arws[1], curses.color_pair(11))
            self.win.addstr(17, 65, arws[2], curses.color_pair(11))
            self.win.addstr(22, 74, arws[3], curses.color_pair(11))
    ...
    def render_opts(self) -> None:
        ...
        for i in range(6):
            ...
            if i == self.idx:
                ...
                self.win.addstr(
                    sy + i, sx_b, f" {" ".join([""] * 32)}", curses.color_pair(3)
                )
    ...
    def increment_carrousel_item(self) -> None:
        ...
        match self.idx:
            case 0:
                self.colorscheme_idx = (self.colorscheme_idx + 1) % len(
                    self.color_schemes
                )
            ...
            case 4:
                self.alt_controls_idx = (self.alt_controls_idx + 1) % len(
                    self.alt_controls_presets
                )
    ...
    def update_setting(self) -> None:
        ...
        match self.idx:
            ...
            case 4:
                settings.update_toml_value("controls", "alt_keys", self.__vals[4])
            case 5:
                settings.update_toml_value(
                    "audio", "enable_sound_effects", self.__vals[5] == "True"
                )
        settings.load()
        settings.set_colors()
